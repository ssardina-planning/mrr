...
public class CausalStructureFactory {
	...
	public static PoclPlan getEquivalentPcoPlan(Plan plan) {
		CausalStructure constraints = getEquivalentPcoConstraints(plan);
		return new PoclPlan(plan.getProblem(), plan.getPlanSteps(), plan.getSubstitution(), constraints);
	}

	public static PoclPlan getMinimalPcoPlan(Plan plan) {
		return getMinimalPcoPlan(plan, false, false);
	}

	public static PoclPlan getMinimalPcoPlan(Plan plan, boolean deorder, boolean ground) {
	...
	public static CausalStructure getEquivalentPcoConstraints(Plan plan) {
		...
		CausalStructure constraints = new CausalStructure(true, true);
	...
	public static CausalStructure getMinimalPcoConstraints(Plan plan, boolean deorder, boolean ground) {
		CausalStructure constraints = new CausalStructure(deorder, ground);
				...
				for (int j = 0; j < (deorder ? i : plan.length()); j++) { // de/reorder -- previous steps/every step in plan 
					...
					if (j == 0) {
						if (pre.getAtom().getSymbol().equals(Predicate.EQUALS)) {
							if (pre.getValue())
								throw new IllegalArgumentException("POSTIVE EQUALITY REQUIREMENT: " + pre);
...
*/
	...
	private static Variable getInitialStateVariable(Variable var, Operator<Variable> initStateOp, Substitution<Constant> sub) {
		for (Variable initVar : initStateOp.getVariables()) {
			if (sub.apply(initVar).equals(sub.apply(var))) {
				return initVar;
			}			
		}
